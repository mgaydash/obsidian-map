...
type ArgValue = string | boolean | number | undefined;

function runObsidian(command: string, args: Record<string, ArgValue> = {}): string {
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    // ── General escape-hatch ───────────────────────────────────────────────
    {
      name: "obsidian",
      description:
        "Run any obsidian CLI command directly. Use this for commands not covered by other tools. " +
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  const a = args as Record<string, ArgValue>;
  ...
  try {
    switch (name) {
      ...
      // ── General escape-hatch ─────────────────────────────────────────────
      case "obsidian": {
        const extraArgs = (a.args as Record<string, ArgValue> | undefined) ?? {};
        return ok(runObsidian(a.command as string, extraArgs));
      }
